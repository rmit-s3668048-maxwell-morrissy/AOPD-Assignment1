...
globals [
  ...
  hours ;; how many ticks/hours have passed in the current day
  infected-today ;; how many infections this current day
  total-infected-yesterday;; how many infections total up to the previous day
]
...
to setup
  ...
  ask patches [ set business? false ]
  construct-businesses
  make-turtles
...
to go
  ...
  if hours = 24 [
    set days days + 1
    set infected-today count turtles with [infected?] - total-infected-yesterday
    set total-infected-yesterday count turtles with [infected?]
    set hours 0]
  tick
end
